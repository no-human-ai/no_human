...
#: "the file is not there" — distinct from any value the file could DECODE to.
#: Needed because THIS refactor centralised the read into `_load`, removing the
#: `is_file()` check each caller used to do for itself. With that check gone,
#: `None` can no longer carry absence: `json.loads("null")` IS `None`, so a
#: present manifest containing `null` would read as no manifest at all — the
#: 89db42ea shape, a real file described to the coder as missing. That never
#: shipped; the sentinel is what keeps it from doing so. (The sibling module
#: `ui_evidence.py` still checks `is_file()` in its callers and so has no such
#: hazard — "use `None`" was never the alternative here.)
_ABSENT = object()


def _load(repo_path: Path) -> tuple[object, str | None]:
    """Read + JSON-decode the manifest file. Returns (data, problem).

    `data is _ABSENT` with no problem means the file is not there. A `problem`
    means the file exists but could not be read or parsed, and `data` is
    `_ABSENT` then too. Any other `data` is what the file decoded to —
    including `None`, for a file whose whole content is `null`.
    """
    ...
    try:
        ...
        return _ABSENT, f"{MANIFEST} is present but is not valid JSON ({exc.msg} at line {exc.lineno})"
    return data, None
...
def manifest_problem(repo_path: Path) -> str | None:
    ...
    Absence is carried by :data:`_ABSENT`, not by ``None``, so a file whose
    whole content is ``null`` is named as a wrong top-level shape rather than
...
def read_manifest(repo_path: Path) -> list[str]:
    ...
    data, problem = _load(repo_path)
    if problem is not None or data is _ABSENT:
        return []
