...
def _env_for(repo_path: Path, env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    # test_cmd is often pytest (this repo's own tests, most attempts): pin its
    # stdout to utf-8 so it agrees with the encoding="utf-8" this module's
    # callers already read that stdout with. A no-op for a non-Python runner
    # (jest/go test/etc.) — none of them read this variable.
