...
def gate(repo, pr_url, base, title, description):
    ...
    caches into ~/.no_human/cache/; and one throwaway local clone per run, in
    its own temp dir, removed on exit (with --pr, the pull request's head is
    fetched straight into that clone, never into THIS checkout). Requires the
    user's own Claude credential (`claude setup-token`), like every `nh` command.
    """
