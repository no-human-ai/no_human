...
def _remove_sandbox(
    ...
    """Remove a sandbox we created. Returns one formatted
    ``"<path>: <ExcType>: <message>"`` string per failure (empty when
    everything went); above the cap the final element is the literal
    ``"... N more"``, which is a count, not a path.
