...
task from a stale one. None of `blockers.wake.WakeWatcher._check_open_pr`'s
existing rungs matched a PR that stayed MERGEABLE while trunk moved past it —
trunk can move repeatedly while a PR sits at AWAITING_APPROVAL, and the only
way back was a fresh coder attempt racing the next landing. This module gives
delivery a base sha to record and the watcher a way to ask, from the real
ref, whether that sha is still the trunk tip.
...
and this rung must never record "fresh" for a base that only passed a
heuristic, only for one verified against the real ref. So `measure()` reuses
`resolve_base_tip` (the ref ladder) but not `overlapping_paths` or
`BASE_STALENESS_REBASE_THRESHOLD` — they are answering "is a rebase worth it
right now", not "is this recorded sha still true", and conflating the two
would let a mergeable-looking gap read as fresh when it was never actually
re-verified.

BACKFILLING A PRE-EXISTING RECORD. A PR delivered before this module existed
carries no `pr_base_sha` at all. The first measurement of such a task must
NOT backfill `observed_sha` (today's trunk tip) as if it were the recorded
base — that would silently assert the PR was built against whatever trunk
happens to be right now, which is false for every PR still open, and a
later FRESH verdict (recorded == observed, trivially true the moment it is
written) would then delete the freshness record outright, leaving the task
permanently indistinguishable from one that was actually just measured
fresh. `merge_base_sha` below answers the honest question instead — the
actual commit the PR's branch forked from — which is knowable for any open
PR and never equals "whatever trunk is today" unless the branch really was
just cut.
"""
...
# Fetches in this module are per-call bounded by a caller-supplied timeout
# (threaded to `_git_rc`) rather than the fixed 120s ceiling, so a watcher
# iterating many parked PRs in one tick can keep each fetch inside a shared
# per-cycle budget. `None` here means "use `_git_rc`'s own default".
DEFAULT_FETCH_TIMEOUT: float | None = None
...
async def fetch_base_ref(repo_path: str, base: str, *,
    ...
    ``timeout`` bounds this ONE call, so a caller polling several PRs in a
    single tick can cap total fetch wall time below the poll interval
    without inheriting `_git_rc`'s fixed 120s ceiling per PR."""
...
async def merge_base_sha(repo_path: str | None, base: str | None,
                          branch: str | None) -> str | None:
    """The actual commit ``branch`` forked from, via ``git merge-base``
    against the resolved trunk tip for ``base``. Used ONLY to backfill a
    pre-existing delivery's `pr_base_sha` honestly (see the module
    docstring) — never to answer the freshness question itself, which stays
    a plain equality check in `measure`.

    Returns ``None`` on anything that prevents a real answer: a missing
    input, an unresolvable base, a pruned/deleted branch, or unrelated
    histories (``git merge-base`` exits non-zero). Never raises."""
    ...
    try:
        ...
        if not tip:
            return None
        rc, sha = await _git_rc(repo_path, "merge-base", tip, branch)
        return sha.strip() if rc == 0 and sha.strip() else None
    except Exception:  # noqa: BLE001 — a watcher rung may never raise
        return None


async def measure(repo_path: str | None, base: str | None,
                   recorded_sha: str | None, *, fetch: bool = True,
                   fetch_timeout: float | None = None) -> BaseFreshness:
    ...
    ``fetch_timeout`` bounds the fetch call alone (see `fetch_base_ref`); it
    has no effect on the local, read-only rev-parse calls beneath
    `resolve_trunk_tip`, which do not touch the network.
    """
    base = base or ""
    ...
    try:
        ...
        if not recorded_sha:
            ...
            # equal" — the caller can backfill from `merge_base_sha` going
            # forward, but the freshness record itself stays undetermined so
            # a backfill is never mistakable for a determined-fresh answer.
