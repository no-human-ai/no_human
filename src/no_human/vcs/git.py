...
class GitRepo:
    ...
    def base_equivalent_commits(self, base_root: str, *, cap: int = 2000) -> set[str]:
        """shas on `HEAD`'s side of `base_root...HEAD` whose PATCH (not
        ancestry) is equivalent to some commit reachable from `base_root` —
        i.e. commits a `git rebase` onto `base_root` re-committed under new
        shas, new author/committer stamps, and a new commit date, but an
        UNCHANGED diff.

        `base_root` MUST already be a sha verified reachable on the real
        remote base — `Orchestrator._base_exclusion_refs` is the only
        caller, and it only ever passes a root it has independently proven
        (the pre-session `base_pin`, or a gate-time read of the *pinned
        remote URL* confirmed present locally via `ensure_remote_commit`).
        This method does no reachability proof of its own: it is a patch
        comparison against whatever root it is handed, so a caller that
        handed it a coder-controlled root (a local branch, a re-pointed
        `remote.origin.url`) would get equivalence against THAT instead —
        the exact laundering shape `_base_exclusion_refs`'s docstring
        describes. Only bare hex (`[0-9a-fA-F]{7,64}`, the same rule
        `commit_identities(exclude=)` already enforces) is accepted; anything
        else raises `GitError` before any subprocess runs.

        Implementation: `git rev-list --count base_root...HEAD` first — if
        the symmetric difference exceeds `cap`, this returns `set()`
        (excuses NOTHING) rather than running an unbounded comparison; a
        multi-thousand-commit history reaching this path is itself a sign
        something is wrong, and staying silent about it would be worse than
        an expensive scan. Then `git rev-list --right-only --cherry-mark
        --no-merges base_root...HEAD`: `--cherry-mark` prefixes each
        HEAD-side (`--right-only`) commit with `=` (patch-id equal to a
        commit on `base_root`'s side) or `+` (no equivalent); only the `=`
        shas are collected. `--no-merges` is deliberate — a merge commit
        the agent itself creates (e.g. merging a refreshed base into the
        branch) must still carry the agent's own identity, never be excused
        by patch-equivalence to whatever it merged.

        RESIDUAL this excuses, by design: a forged-identity commit whose
        diff happens to be byte-identical to a commit the real base already
        has is excused. That introduces no content the base doesn't already
        have — the attacker has reproduced base content, not smuggled
        anything new — so excusing it costs nothing a laundering attack
        could exploit.

        Any non-zero exit or line that doesn't parse as `rev-list
        --cherry-mark` output raises `GitError` — never a silent `set()` for
        a read that actually failed; that distinction is the caller's to
        make (mirrors `commit_identities`)."""
        if not re.fullmatch(r"[0-9a-fA-F]{7,64}", base_root):
            raise GitError(
                f"base_equivalent_commits(): not a hex sha: {base_root!r}"
            )
        count_out = self._run("rev-list", "--count", f"{base_root}...HEAD")
        try:
            total = int(count_out.strip())
        except ValueError:
            raise GitError(
                f"base_equivalent_commits(): unparseable `rev-list --count` "
                f"output: {count_out!r}"
            )
        if total > cap:
            log.warning(
                "base_equivalent_commits(%s): symmetric difference %d "
                "exceeds cap %d; excusing nothing (fail closed)",
                base_root, total, cap,
            )
            return set()
        out = self._run(
            "rev-list", "--right-only", "--cherry-mark", "--no-merges",
            f"{base_root}...HEAD",
        )
        ...
        for line in out.split("\n"):
            if not line:
                continue
            mark, sha = line[0], line[1:]
            if mark not in ("=", "+") or not re.fullmatch(r"[0-9a-fA-F]{7,64}", sha):
                raise GitError(
                    f"base_equivalent_commits(): unparseable `rev-list "
                    f"--cherry-mark` line: {line!r}"
                )
            if mark == "=":
                equivalent.add(sha)
        return equivalent
    ...
    def ensure_remote_commit(self, remote: str, branch: str, sha: str, *,
                              timeout: int = 30) -> bool:
        """Public wrapper over `_have_remote_commit`: proves *sha* — a value
        already read from `remote` via `ls_remote_exact` — is actually
        present in this local object store (fetching it into a private
        namespace on a miss, never the tracking ref), not merely a name
        `ls-remote` advertised. No behaviour change from `_have_remote_commit`
        itself; this exists so a caller like
        `Orchestrator._base_exclusion_refs` can verify a gate-time remote
        read before trusting it as an exclusion root, without reaching into
        a private method.

        Fails CLOSED: any exception (timeout, git missing) or a fetch/
        cat-file failure returns `False`, never raises — the caller's
        response to `False` is to drop the candidate root, which means MORE
        commits stay in the attribution window, never fewer."""
        try:
            return self._have_remote_commit(remote, branch, sha, timeout)
    ...
    def ls_remote_exact(self, ref: str, *, remote: str = "origin",
        ...
        argv as an option. `remote` is checked the same way: a THIRD
        gate-time caller (`Orchestrator._base_exclusion_refs`, resolving
        the pinned-remote-URL exclusion root) passes a `remote` value that
        was captured once, pre-session, in the orchestrator's own frame —
        never coder-writable — but validating it here too means even a
        future caller that forwards an untrusted string can't smuggle an
        option through the `remote` slot either. Every other unreadable
        state git can hand back — rc != 0 (auth/unreachable/bad URL/unknown
        ref), empty stdout, a timeout, `OSError` (git missing), or
        two-or-more DISTINCT shas both exactly matching `ref` (should not
        happen for a single ref, but is treated as ambiguous rather than
        guessed at) — returns `None`. This is the pin `Orchestrator._run_attempt`
        reads ONCE, before the coder session starts; a caller that re-reads
        this at gate time instead of trusting the pin would let a
        force-push after pinning move the answer, which is exactly the
        laundering path this method exists to avoid. `fetch_remote_branch_sha`
        is a second, gate-time caller of this same method — that is safe
        despite the warning above because delivery only ever pushes a sha
        that has already been proven equal to a PASS-stamped review sha
        (see `_assert_delivery_sha`); there is no unreviewed history for a
        gate-time re-read to launder in.
        """
