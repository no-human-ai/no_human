...
is NOT used: it is still sandboxed and its production-access request was
denied (``ReviewDetails.Status: DENIED``, case 178688493800757), so in
sandbox it can only reach a handful of individually verified addresses rather
than an arbitrary registrant.

What is configured, and what an unconfigured install does. ``ResendTransport``
is selected by ``_default_transport`` ONLY when ``RESEND_API_KEY`` is present
in ``~/.no_human/.env`` (chmod 600, gitignored — never config.yaml, the same
rule ``_reject_api_key_in_config`` enforces for the Anthropic key). Without it
the default is still ``UnavailableTransport``: registration succeeds,
``send_welcome`` returns ``"not_sent:unconfigured"``, and nothing touches the
network. The egress is declared in ``tests/test_egress_allowlist.py`` under
``email/send.py``, gated ``env:RESEND_API_KEY``.

Mail leaves from ``send.getnohuman.com``, never the root: getnohuman.com runs
Cloudflare Email Routing (MX route1/2/3.mx.cloudflare.net), and a sender on
the root would break inbound mail to support@getnohuman.com.
...
Swapping the transport is still a ONE-MODULE change: implement
``Transport.send`` and return it from ``_default_transport``.
"""
...
#: The SENDING subdomain, never the root: getnohuman.com already runs
#: Cloudflare Email Routing (MX route1/2/3.mx.cloudflare.net), and putting the
#: sender on the root would break inbound mail to support@getnohuman.com.
RESEND_SENDER = "Eyal from no_human <eyal@send.getnohuman.com>"
#: Replies go to a human inbox, not the sending subdomain.
RESEND_REPLY_TO = "eyal@getnohuman.com"
#: Read from ~/.no_human/.env (chmod 600, gitignored) exactly like every other
#: credential here. NEVER config.yaml — `_reject_api_key_in_config` is the
#: rule this follows — and never logged, never in an exception message.
...
#: HTTP status -> (category, retryable). Anything unlisted is a transport
#: error: if we cannot establish that the provider ACCEPTED the message, it
#: was not sent. Never map an unknown status to success.
...
def _resend_api_key(env_path=None) -> str | None:
    ...
    `UnavailableTransport` and behaving exactly as before. Imports `config`
    locally (not at module top) so a test's `monkeypatch.setattr` on
    `no_human.config.ENV_PATH` is honored even though this module is imported
    once at startup.
    """
...
class ResendTransport:
    ...
    No `resend` SDK and no `requests`: the lean-stack constraint forbids
    adding a dependency for this, and the whole call is one POST.

    Nothing here ever puts the recipient in a log line or in a raised
    message. Resend's error bodies can echo the address, so the body is read
    only to pick a CATEGORY out of the closed set above; the body itself is
    discarded.
    """

    def __init__(self, api_key: str, *, sender: str = RESEND_SENDER,
                 reply_to: str = RESEND_REPLY_TO, endpoint: str = RESEND_ENDPOINT,
                 timeout: float = 10.0, opener=None) -> None:
        self._key = api_key
        self._sender = sender
...
def _platform_template(platform: str | None) -> str:
    ...
    `platform` None means THE HOST: it falls back to `sys.platform`, so a
    Linux install renders the Linux template and a Windows install the
    Windows one. Measured: with `platform=None`, sys.platform 'linux' ->
    linux_download, 'win32' -> windows_waitlist, 'darwin' -> mac_download.
    macOS is only the fallback for an UNRECOGNIZED platform string.
    Production calls this with None (`send_welcome` in `api/app.py`).
    """
