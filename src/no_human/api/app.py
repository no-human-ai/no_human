...
# `BASE_STEPS` and `telemetry.ONBOARDING_STEPS`. Kept as a plain tuple (not
# imported from the frontend, which the server cannot do) and pinned equal to
# both, and to onboardingFunnel.js's FUNNEL_STEPS, by
...
# trusting this tuple to stay in sync on its own.
_WIZARD_STEPS = ("welcome", "email", "repos", "projects", "integrations", "discord", "summary")
...
def _well_formed_email(addr: str) -> bool:
    r"""Basic well-formedness: one "@", non-empty local and domain parts, and a
    domain that itself looks like a domain (contains a dot, does not start or
    end with one). Agrees with `isWellFormedEmail` in
    web/src/onboardingEmail.js for every case the shared table covers, but the
    two are NOT byte-identical on whitespace: JS `\s` includes U+FEFF and
    Python `str.isspace()` does not, so an interior U+FEFF is rejected by the
    client and accepted here. This side is authoritative and more permissive,
    so the divergence cannot admit anything the client would have blocked.
    The server re-checks because the client gate is bypassable and the wizard
    must not trust it."""
    s = (addr or "").strip()
