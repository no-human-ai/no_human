...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "mutation_probe": {
        ...
        # "advisory" | "required". A SURVIVED test (green under a mutation
        # of its own named behaviour) always blocks, in either mode — that
        # is not an advisory-severity fact. This knob only controls a probe
        # that COULD NOT run (non-Python test, unmappable, missing
        # interpreter, budget exhausted, crash): recorded either way, but
        # only blocks in "required". Scope: only tests a diff adds or
        # changes, Python/pytest only — never a repo-wide mutation score.
        "mode": "advisory",
        # Cap on changed/added tests probed per review (one pytest launch
        # per test per mutation) and mutation candidates tried per test
        # before giving up and reporting "survived".
        "max_tests": 12,
        "max_mutations_per_test": 3,
        # Wall-clock budget in seconds for the whole run; exceeding it
        # reports the remainder "undetermined", never a silent pass.
        "timeout_seconds": 300,
    },
    ...
    "hooks": {
        ...
        "per_edit_lint": True,
        # Per-edit type feedback (agent/type_hook.py, issue #114 phase 2): after
        # each Edit/Write to a .py/.pyi file, run the checker the repo already
        # configures over that one file and feed back what it did not report
        # before the edit. OFF by default, unlike per_edit_lint: a type check is
        # an order of magnitude dearer than a ruff call, so this ships advisory
        # and opt-in until dogfood numbers say what it costs per attempt. No-op
        # unless the repo configures pyright or mypy; `tsc` is excluded because
        # a per-file run ignores tsconfig.json. Bounded twice — per run
        # (TYPE_HOOK_TIMEOUT) and per attempt (TYPE_HOOK_SESSION_BUDGET) — and
        # silent on any checker failure rather than reporting a false clean.
        "per_edit_type": False,
    },
    "docs": {
