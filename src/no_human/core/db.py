...
class Store:
    ...
    async def abandon_open_attempt(
        ...
        """Close *task_id*'s open attempt row as ``interrupted`` with its
        usage columns made non-NULL, for a caller (the stall watchdog) that
        is escalating a task while its worker may STILL be running.

        Unlike `close_open_attempts` — called only by checkpoint-CLEARING
        paths on a task whose worker is already gone — this can run while
        the process that opened the row is still alive (Criterion 4: the
        stall sweep never cancels the backend coroutine). That is safe
        because the row is selected and updated by id, same as every other
        `update_attempt` write; if the live session later finishes on its
        own, its own `update_attempt` legitimately overwrites this row with
        the true final numbers, same as any other race between two writers
        of one row. These numbers are last-observed telemetry, not a final
        result.

        Picks the newest OPEN row the same way `latest_open_attempt` does —
        `(started_at DESC, rowid DESC)`, not `attempt_number` (see that
        method's docstring for why a lower number can be written later).
        Returns the closed row (post-update), or None if there was no open
        row — a second sweep pass over an already-escalated task is then a
        pure no-op, which is what makes `_escalate_if_stalled` idempotent.

        The five usage columns are `COALESCE(<col>, 0)`, not overwritten with
        a fixed 0 — anything the run already billed onto the row before it
