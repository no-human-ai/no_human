...
def test_commit_identities_blocks_injection(tmp_path):
    ...
    repo = GitRepo(work)
    with pytest.raises(GitError):
        repo.commit_identities("main", exclude=["--upload-pack=evil"])
    with pytest.raises(GitError):
        repo.commit_identities("main", exclude=["-oProxyCommand=evil"])
    with pytest.raises(GitError):
        repo.commit_identities("main", exclude=["not-hex-at-all"])
