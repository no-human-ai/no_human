...
async def test_the_conflict_event_text_tells_the_coder_to_merge_not_rebase(
    ...
    # `base_pin` is threaded through when the caller resolved one; either
    # way the event text must carry SOME merge target, never silence.
    if staleness.get("base_pin"):
