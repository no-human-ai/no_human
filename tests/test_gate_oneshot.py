...
def test_run_gate_does_not_materialize_no_human_home_on_a_fresh_machine(
    ...
    """The same guarantee through `run_gate` itself: on a machine that has
    never run `nh init`, `run_gate` must not create `~/.no_human` as a side
    effect of its read-only config lookup, and must not write an empty
    `config.yaml`. Scope, so the name is not read too widely: this drives
    `run_gate`, not the `nh gate` CLI verb, so it cannot see the CLI group's
    background update check, which does cache into `~/.no_human/cache/` —
    the surface `SKILL.md` documents."""
    from no_human import config as config_mod
