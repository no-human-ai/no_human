...
FROZEN_FILE_LINES = {
    ...
    # 21502 -> 21615 (+113): attribution guard hardening — the base-sha pin
    # capture in `_run_attempt` (docstring above the call site plus the
    # fail-closed advisory/log branch), `_base_exclusion_refs` and its
    # `base_pin`-aware paragraph on `_foreign_authored_commits`'s docstring,
    # and the `_protect_base_branch` hook-refresh call. Measured on this
    # tree with the scanner below, never summed.
    "core/orchestrator.py": 21615,
    ...
    # 5052 -> 5062 (+10): `base_pin_sha` added to `Store._ensure_task_columns`'s
    # `wanted` dict (attribution guard hardening) plus its explanatory
    # comment. Measured on this tree with the scanner below.
    "core/db.py": 5062,
