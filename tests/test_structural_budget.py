...
FROZEN_FUNCTION_LINES = {
    ...
    # to this task's own diff.
    # 2274 -> 2280 (+6): merge of main's independent citation-drift preflight
    # feature (the `_citation_drift_preflight` invocation immediately after
    # the structural-budget preflight block, wrapped in the same
    # `try/except CancelRequested` / `except (BudgetAbort, StuckAbort,
    # ConvergenceAbort)` pattern the two sibling preflight call sites already
    # use) into this branch.
    # 2280 -> 2293 (+13): merge of origin/main (e85feb29) into this branch —
    # both parents had independently grown `_run_attempt` since they last
    # shared a base, so the +6 above (this branch's own merge of an earlier
    # main) and main's further growth do not simply add; the value below is
    # not derived by arithmetic but re-measured directly on the merged tree
    # with the scanner's own metric:
    ...
    "core/orchestrator.py:Orchestrator._run_attempt": 2293,
...
FROZEN_FUNCTION_CC = {
    ...
    # 252 -> 255 (+3): merge of main's independent citation-drift preflight
    # call site — the `try/except CancelRequested` / `except (BudgetAbort,
    # StuckAbort, ConvergenceAbort)` wrapper around the
    # `_citation_drift_preflight` call adds one `try` handler branch each.
    # Re-measured on the merged tree with the scanner below.
...
FROZEN_FILE_LINES = {
    ...
    # 24296 -> 24364 (+68): merge of main (0b8c2dc4, "A pushed task branch
    # is never rewritten, and the guard's claims are executable") into this
    # branch — main's independent pushed-tip-rewrite-guard growth on this
    # same file (its own history: 24073 -> 24133 -> 24141, i.e. +68 on top
    # of a pre-divergence 24073 baseline), reconciled with this task's own
    # 24296 tip. Re-measured on the merged tree by the scanner's own metric:
    #   uv run python -c "import sys; sys.path.insert(0,'tests');
    #   from test_structural_budget import scan_tree, SRC;
    #   _,_,files,_,_ = scan_tree(SRC);
    #   print(files['core/orchestrator.py'])"
    ...
    #   print(files['core/orchestrator.py'])"
    # -> 24375.
    # 24375 -> 24396 (+21): merge of main's independent history (see the
    # parallel entry below, which traces main's own citation-drift-preflight
    # feature from a 24073 baseline up to 24728) reconciled with this task's
    # own 24375 tip. This branch's side of the reconciliation stopped being
    # separately re-derivable once both histories were folded into one tree;
    # the number below is the actual merged-tree measurement, not carried
    # arithmetic from either parent.
    # 24396 -> 24728: folding in main's remaining citation-drift-preflight
    # growth (its own history immediately below, 24073 -> 24728) on top of
    # this branch's 24396. Re-measured on the actual merged tree with the
    # scanner's own metric:
    #   uv run python -c "import sys; sys.path.insert(0,'tests');
    #   from test_structural_budget import scan_tree, SRC;
    #   _,_,files,_,_ = scan_tree(SRC);
    ...
    # 24728 -> 24983 (+255): merge of origin/main (e85feb29) into this
    # branch — another round of independent main-branch growth in this
    # file, conflicting in this file's own frozen table (this entry) and
    # resolved the same way as the 24606 -> 24674 merge above: by
    # re-measuring THIS tree (the actual post-merge write-tree result)
    # with `scan_tree` rather than adding either parent's delta by hand.
    "core/orchestrator.py": 24983,
