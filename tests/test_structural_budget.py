...
FROZEN_FILE_LINES = {
    ...
    # one. Measured on this tree with the scanner below.
    "cli/commands.py": 8462,
    ...
    # 5919 -> 5989 (+70): setup-mode boot — `_require_credentials` (the
    ...
    # setup-state computed in `lifespan` and stored on `app.state`, and
    # `setup_mode` added to `GET /api/config`. The +10 on top of the first
    # cut (5979) is `_require_credentials`/`show_config` gating on
