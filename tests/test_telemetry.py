...
def test_client_allowlist_matches_the_deployed_lambda_contract():
    ...
    `task_ended`/`tasks_orphaned` and the four onboarding-funnel events are
    valid client-side (`_ALLOWED_EVENTS`, PostHog gets them) but are NOT yet
    in this deployed contract — they are kept out of the outgoing Lambda
    batch by `_LAMBDA_EVENTS` (see telemetry.py, docs/TELEMETRY.md) until the
    server ships them.
    """
    ...
    # back, and none of the new events are silently forgotten either.
