"""A resumed attempt's `[WIP-BLOCKED]` head must reach a reviewer, never a
zero-diff terminal — both incidents, 2026-09-08.
...
checkpoint could reach either one with an unjudged diff still sitting at
head:
...
reproduces both incidents, pins the routing across every `resume_from.by`
...
async def test_the_same_routing_applies_to_every_resume_provenance(
    ...
    ineligible through `_head_is_blocked_checkpoint` (the head's own shape);
...
async def test_unreadable_head_and_unreadable_subject_fail_closed(tmp_path, store):
    ...
    2. head IS readable, `commits_ahead` raises -> assumed a diff exists (the
       ...
       `_head_is_blocked_checkpoint` must assume the unsafe (blocked) side,
       not the safe one.
    """
    orch = Orchestrator(store, _config(tmp_path).data,
                        ScriptedBackend(lambda cwd: _ok("noop")), SlackNotifier(None))
    ...
    assert not eligible, why
    assert orch._head_is_blocked_checkpoint(_HeadOnly(), "deadbeef") is True
...
async def test_already_satisfied_eligibility_table(tmp_path, store):
    ...
    iff there IS an unjudged diff AND (the head is a `[WIP-BLOCKED]`
    checkpoint OR the provenance is a machine requeue). Every other
    combination — no diff at all, an ordinary subject on a non-machine
    provenance (D15), a `[WIP-PARTIAL]` subject on `wake` (`test_server_stop_
    checkpoint.py`) — stays eligible."""
    orch = Orchestrator(store, _config(tmp_path).data,
                        ScriptedBackend(lambda cwd: _ok("noop")), SlackNotifier(None))
    ...
    cases = [
        ...
        ("partial-wake", "[WIP-PARTIAL] partial", "wake", True),
