...
# A SECOND structurally-real PNG whose bytes differ from PNG_1x1 -- it is 2x1,
# so a test can tell "the page changed" from "the same frame was captured
# again" without either being a made-up blob. (The name says 1x1 for symmetry
# with the constant above; the IHDR says 2x1. Kept rather than renamed because
# the name appears in several tests and the dimensions are irrelevant to every
# one of them -- only "differs from PNG_1x1" matters.)
PNG_1x1_ALT = (
    b"\x89PNG\r\n\x1a\n"
    + b"\x00\x00\x00\x0d"
    + b"IHDR"
...
#: A THIRD structurally-real PNG, so a walk can have three genuinely distinct
#: frames without reusing either of the two above.
PNG_2x2 = (
    b"\x89PNG\r\n\x1a\n"
    + b"\x00\x00\x00\x0d"
    + b"IHDR"
...
def test_the_omitted_count_counts_shots_not_runs(tmp_path, monkeypatch):
    """`(+N more shot(s))` counts SHOTS the reader cannot see, not runs.

    This is the one existing number this change redefined -- it was
    `len(delivered) - cap` and is now the sum of the shot names inside
    dropped runs -- and nothing in the repo tested it. A mutation returning
    `len(dropped)` (runs instead of shots) left the entire file green.

    The two only differ when a DROPPED run holds more than one shot, so the
    fixture puts a three-shot repeat past the cap: counting runs would say 1,
    counting shots says 3. A reader deciding whether to open the artifact
    needs to know how many things they are not being shown, not how many
    groups those things fell into.

    The first six runs are built by ALTERNATING two images rather than by
    finding six distinct ones -- runs group consecutively, so A,B,A,B,A,B is
    six runs from two files. That construction also catches a mutation that
    groups against any earlier run rather than only the previous one, since
    global grouping would collapse the alternation to two runs.
    """
    cap = Orchestrator._UI_EVIDENCE_MAX_EMBEDDED_SHOTS
    frames = []
    for i in range(cap):
        frames.append((f"step{i}", PNG_1x1 if i % 2 == 0 else PNG_1x1_ALT))
    frames += [("tail-a", PNG_2x2), ("tail-b", PNG_2x2), ("tail-c", PNG_2x2)]
    section = _section_for_shots(tmp_path, monkeypatch, frames)

    assert section.count("![") == cap, (
        f"expected exactly {cap} embeds (the cap), got "
        f"{section.count('![')}: {section}")
    # Anchored on the `+`: an unanchored "3 more shot(s)" also matches
    # "+13 more shot(s)". Unreachable on this fixture, where omitted maxes
    # at 3 -- but the anchor costs one character and removes the class.
    assert "+3 more shot(s)" in section, (
        "the omitted count must report the three SHOTS inside the dropped "
        f"run, not the one run: {section}")
    assert "+1 more shot(s)" not in section, section
...
def test_frames_that_really_differ_each_keep_their_own_embed(
        tmp_path, monkeypatch):
    """Three DISTINCT frames stay three embeds.

    Positive control for the collapse: it must key on the BYTES. An earlier
    revision of this file paired this with an A/B/A case whose shot sequence
    was byte-identical to it -- two tests, one input, so they passed and
    failed together and the second bought nothing.
    """
    section = _section_for_shots(tmp_path, monkeypatch, [
        ("landing", PNG_1x1), ("expanded", PNG_1x1_ALT), ("final", PNG_2x2)])
...
def test_a_frame_repeating_after_the_page_changed_is_still_its_own_step(
        tmp_path, monkeypatch):
    """A->B->A is three observations: it changed and changed back.

    Only a CONSECUTIVE repeat is a step that captured nothing new, so this
    must not collapse to two embeds. Distinct from the all-distinct case
    above: here the first and third frames really are the same bytes.
    """
    section = _section_for_shots(tmp_path, monkeypatch, [
...
def test_two_separate_runs_each_collapse_on_their_own(tmp_path, monkeypatch):
    """A,A,B,B is two observations, not one and not four."""
    section = _section_for_shots(tmp_path, monkeypatch, [
        ("a1", PNG_1x1), ("a2", PNG_1x1), ("b1", PNG_1x1_ALT), ("b2", PNG_1x1_ALT)])

    assert section.count("![") == 2, section
    assert section.count("byte-identical") == 2, section


def test_the_cap_is_spent_on_distinct_frames_not_on_repeats(
        tmp_path, monkeypatch):
    """Grouping happens BEFORE the cap, so repeats cannot crowd out states.

    With the cap applied to the shot list first, a walk of six identical
    frames followed by two genuinely different page states rendered one
    embed and hid both distinct states behind "+2 more shot(s)" -- with five
    embed slots still free. Measured; that ordering is what this pins.
    """
    frames = [(f"idle{i}", PNG_1x1) for i in range(6)]
    frames += [("expanded", PNG_1x1_ALT), ("final", PNG_2x2)]
    section = _section_for_shots(tmp_path, monkeypatch, frames)

    assert section.count("![") == 3, section
    assert "expanded" in section and "final" in section
    assert "more shot(s)" not in section, section


def test_the_note_claims_only_what_the_bytes_establish(tmp_path, monkeypatch):
    """Identical pixels are not proof the page did not change.

    A save-then-reload walk is the counter-example that matters: the two
    captures being identical IS the persistence evidence, so a note reading
    "the page did not change between them" would describe that walk exactly
    backwards.
    """
    section = _section_for_shots(tmp_path, monkeypatch, [
        ("after-save", PNG_1x1), ("after-reload", PNG_1x1)])

    assert "byte-identical image" in section, section
    assert "did not change" not in section, section
    assert "one observation" not in section, section
