...
spent on drift the coder could not have known about at review time.
...
async def test_the_mechanical_fix_commit_reports_a_manifest_repair_not_silently(
    ...
    """`commit_with_manifest_repair` can mutate the pin ledger, so a call
    site that does not pass `on_repair` drops the repair unreported and a
    re-approved pinned file the commit incidentally touched never reaches
    the task's event record. `_checkpoint_commit` and the pipeline commit
    drain it; the citation drift preflight's own mechanical-fix commit did
    not, and now does. The corrective round's call site still does not —
    a known gap, not closed here. Proven behaviourally: `orch_mod.commit_with_manifest_repair`
    ...
    assert "on_repair" in seen_kwargs, (
        "the citation drift preflight's commit must pass on_repair, the "
        "way _checkpoint_commit and the pipeline commit do"
    )
...
async def test_an_unknown_run_with_a_partial_write_is_reverted_before_the_round(
    ...
    Also pins the advisory's wording (send-back finding): the citation
    drift preflight's revert is its OWN mechanical
