...
TIMING, MEASURED, BECAUSE "BOUNDED" READS AS "QUICK" AND IS NOT.
`_LEASE_WRITE_ATTEMPTS`/`_LEASE_WRITE_BACKOFF_S` bound only the SLEEP between
attempts (0.15s in total). Each ATTEMPT first sits in SQLite's own busy
handler: the store opens with `PRAGMA busy_timeout = 5000`, so a contended
refresh costs roughly that per attempt. Against a foreign `BEGIN IMMEDIATE`
held throughout, the honest `PoolLeaseLost` refusal took 5.46s before the
retry existed and 16.21s after it -- one contended refresh blocks `tick()`
about three times longer than it used to. Still far under the 60s
`tick_stalled` threshold, and still fail-closed, but not cheap.

That same timeout narrows where the retry helps at all: at
`busy_timeout = 5000` a MOMENTARY lock never raises, because the busy handler
absorbs it. The reachable window is a foreign write lock of roughly 5-15s.
Every executed lock test in this file sets `busy_timeout = 0`, so none of them
covers that production timing -- stated here rather than left for the next
