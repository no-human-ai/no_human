...
Task c4873936 REFILE: main added "email" and "discord" steps to
Onboarding.jsx's BASE_STEPS after this file's original step lists were
written, so the server/telemetry/frontend step mirrors above silently
dropped both from the funnel. `test_step_key_matches_the_wizards_own_list`
below now parses Onboarding.jsx's own BASE_STEPS source (not a hand-copied
literal) so a future drift like this fails loudly instead of silently.
"""
...
def test_step_key_matches_the_wizards_own_list():
    ...
    exactly how "email" and "discord" went unreported: BASE_STEPS grew two
    steps and nothing here noticed)."""
    jsx_path = Path(__file__).resolve().parent.parent / "web" / "src" / "Onboarding.jsx"
    jsx_src = jsx_path.read_text(encoding="utf-8")
    m = re.search(r"const BASE_STEPS = \[([\s\S]*?)\n\];", jsx_src)
    assert m, "BASE_STEPS literal not found in Onboarding.jsx"
    wizard_steps = tuple(re.findall(r'\{\s*key:\s*"([^"]+)"', m.group(1)))
    assert wizard_steps, "expected at least one { key: \"...\" } entry in BASE_STEPS"

    assert tuple(_WIZARD_STEPS) == wizard_steps
    assert telemetry.ONBOARDING_STEPS == set(wizard_steps)

    js_path = Path(__file__).resolve().parent.parent / "web" / "src" / "onboardingFunnel.js"
    js_src = js_path.read_text(encoding="utf-8")
    fm = re.search(r'FUNNEL_STEPS\s*=\s*\[([^\]]*)\]', js_src)
    assert fm, "FUNNEL_STEPS literal not found in onboardingFunnel.js"
    js_steps = [s.strip().strip('"').strip("'") for s in fm.group(1).split(",") if s.strip()]
    assert tuple(js_steps) == wizard_steps  # order matches too
...
# =========================================================================== #
# Merge hazard: main's email redaction on /api/onboarding/complete must      #
# survive alongside the new "completed" telemetry marker.                    #
# =========================================================================== #

@pytest.mark.asyncio
async def test_complete_response_never_echoes_a_registered_email(client, tmp_path):
    """PR #302's onboarding_complete returned the raw onboarding block; main
    later added `_onboarding_public()` redaction so a registered email is
    never echoed back. The merge must keep main's redaction while still
    emitting `onboarding_completed` -- this fails if either regresses."""
    r = await client.post(
        "/api/onboarding/email", json={"email": "someone@example.com"}
    )
    assert r.status_code == 200, r.text
    ...
    assert r.status_code == 200, r.text
    body = r.json()
    assert "someone@example.com" not in repr(body)
    assert "email" not in body.get("onboarding", {})
