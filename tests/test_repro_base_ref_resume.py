...
async def test_is_resume_shape_false_for_a_plain_task(store, tmp_path):
    orch = _orch(store, tmp_path)
    task = Task.new("t", repo_path="/r")
    assert orch._is_resume_shape(task) is False


async def test_is_resume_shape_true_for_explicit_context_flag(store, tmp_path):
    orch = _orch(store, tmp_path)
    task = Task.new("t", repo_path="/r")
...
async def test_is_resume_shape_true_when_resume_from_sha_present(store, tmp_path):
    orch = _orch(store, tmp_path)
    task = Task.new("t", repo_path="/r")
...
async def test_is_resume_shape_false_on_missing_or_malformed_context(store, tmp_path):
    orch = _orch(store, tmp_path)
    task = Task.new("t", repo_path="/r")
    task.context = None
    assert orch._is_resume_shape(task) is False
...
async def test_resume_base_walks_past_multiple_attempt_commits(store, tmp_path):
...
async def test_resume_base_never_returns_a_wip_checkpoint_when_identity_differs(store, tmp_path):
...
async def test_all_agent_authored_history_returns_an_unresolvable_base(store, tmp_path):
...
async def test_non_resume_fallback_is_still_head_tilde_one(store, tmp_path):
...
async def test_merge_base_still_wins_when_base_resolves(store, tmp_path):
...
async def test_51eeaa07_shape_end_to_end(store, tmp_path):
