...
Re-anchored again 2026-09-03 (fourth): the WIP-checkpoint resume-digest
    ...
    Re-anchored again 2026-09-08 (round 5): that version's predicate read
    task.context["review_history"], which is False on the real incident
    (task 82644133's attempts never carry a review_history[].at entry).
    `_send_back_resume_round` is rewritten to key directly on the
    `attempts` table (`review_passed`, `commit_sha`, `started_at`)
    instead, and its docstring grows to explain why both the round-2
    committer-date rule and the round-3 review_history rule failed closed
    on the incident. `_land_no_changes_needed`'s docstring and `detail`
    string were also reworded to match. Net +15 lines above
    `_run_attempt`'s `update_attempt(attempt_id, branch_name=branch)`
    call, moving the citation from 4930 to 4945; re-verified against the
    code, not carried forward blind. db.py:2306 is untouched by this
    change.
    """
    ...
    assert "orchestrator.py:4945" in known_issues_doc, (
        "the traceback no longer cites orchestrator.py:4945 - this test is "
    ...
    assert 1 <= 4945 <= len(orch_lines), "orchestrator.py is now shorter than line 4945"
    assert "self.store.update_attempt(" in orch_lines[4944], (
        f"orchestrator.py:4945 is now {orch_lines[4944]!r}, not the "
        f"update_attempt call the traceback names"
    )
    assert "self.store.update_attempt(" in orch_body, (
        "line 4945 is no longer inside _run_attempt's body"
    )
