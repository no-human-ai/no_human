...
Re-anchored again 2026-09-03 (fourth): the WIP-checkpoint resume-digest
    ...
    Re-anchored again 2026-09-08 (no-op send-back resume, rebased onto
    current main): `_send_back_resume_round` and `_land_no_changes_needed`
    (rounds 1-3 introduced the feature; round 5 replaced round 3's
    `task.context["review_history"]` read with a predicate keyed directly
    on the `attempts` table — `review_passed`, `commit_sha`, `started_at`
    — because the real incident's rows never carry a `review_history[].at`
    entry) together add 138 net lines above `_run_attempt`'s
    `update_attempt(attempt_id, branch_name=branch)` call in
