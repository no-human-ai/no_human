...
stub (pattern from `tests/test_flaky_rerun_attribution.py`'s `_stub`):
  ...
  * green serial re-run -> not counted against the change, attempt proceeds
    exactly as if the TESTING run itself had been green (AC1a).
...
async def test_serial_rerun_green_lets_the_attempt_proceed(
    ...
    ONLY the failing files is not counted against the change."""
    scripted = [
    ...
    named = [e for e in tests_events if e.get("serial_rerun_passed")]
    assert named, [e for e in tests_events]
...
async def test_pytest_command_is_never_serially_rerun(
    ...
    """AC1c: a plain pytest command never takes the node serial-rerun path —
    only the pre-existing (out-of-scope) pytest machinery may run again."""
