...
async def test_serial_rerun_still_red_fails_as_today(
    ...
    # The ORIGINAL (concurrent) result is what flows on — never the serial
    # re-run's counts (passed=1, failed=1) or its narrower failing list (ID1
    # only): the fixture's concurrent run is passed=0, failed=1 with both ids.
    assert sorted(tr["failing_tests"]) == sorted([ID1, ID2])
    assert tr["failed"] == 1 and tr["passed"] == 0
