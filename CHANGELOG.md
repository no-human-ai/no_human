...
- **The board can now tell you about an update at all, and a late-mounting
  window no longer misses one.** Nothing about an update was ever retained
  before — there was no memory of a startup check at all, so a window that
  mounted (or remounted) after one finished stayed notice-free for the rest
  of the session. A version FACT (available/unavailable/up-to-date) is now
  retained and handed to any surface that pulls it on mount; an automatic
  (app-driven) check that merely *fails* is deliberately excluded from that
  memory, so a launch with no network does not resurrect a stale "Could not
  check for updates" card later — a *manual* "Check for updates" still shows
  its own failure immediately, unchanged. Separately, the task board itself
  now shows a small in-flow notice when an update is available or can't
  auto-install — it never showed anything before; only Settings did —
  rendered as a normal document-flow element after the top bar rather than
  overlaid on top of it. Clicking "Later", from Settings or from the board,
  persists a defer for that version: later *automatic* checks stay silent
  for it permanently (a manual check still answers), and the click also
  clears the board's own copy of the notice immediately.
