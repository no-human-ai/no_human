...
_Harness-captured record for task `beedca29`, commit `880c91ddd9fc041cb75f6d395929571fc32d0e4b` — not model-authored: no_human wrote this file from the repo's merge policy evaluated against this commit — advisory to the human, nothing merges on it. It records what the gate produced; it is not a verdict of the model that wrote the code._
...
```json
{
  ...
  "rules": [
    ...
    {
      "detail": "tests: 0 failed of 12992 run",
    ...
    {
      "detail": "2 verifiers, none failed (1 no verdict (advisory): no-unvalidated-status-write)",
