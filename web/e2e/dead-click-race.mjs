...
async function runPass(browser, variant) {
  ...
  for (const scenario of ["rerender-button", "dead-button"]) {
    ...
    // Force any still-queued request out now rather than waiting the rest of
    // the flush timer — `_handle_unload` is the real, stable (non-minified)
    // method posthog-js itself binds to the page's pagehide/unload event, so
    // this mirrors what a real tab close does rather than reaching into
