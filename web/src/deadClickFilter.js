...
// falls in that race window. A genuine timeout with a large gap or a present
// mutation delay is untouched — but the window is a heuristic, not a proof:
// a genuinely dead control clicked within 100 ms of ANY unrelated page-wide
// mutation is dropped too, because `$dead_click_last_mutation_timestamp` is
// the page-wide last-mutation stamp, not this element's. Measured in the
// harness: a dead button clicked ~30 ms after an unrelated mutation is
// filtered out (observed gaps 43 and 49 ms). The margin is why the window is
// still worth it — genuine dead controls measured 1216-1420 ms in the harness
// and 2590/3191 ms in the live data, 12x to 32x the window. The mutation that
// lines up need not be periodic: the most reachable source is any preceding
// interaction that re-renders (a click on a live control, then one on a dead
// control within 100 ms), with the app's own ticks (Board.jsx's 60 s,
// App.jsx's 10 s poll) as the background sources. This does NOT cover
// `capture_heatmaps`'s own dead-click layer — see telemetry.js for that
// disclosure.
...
// [0, DEAD_CLICK_RACE_WINDOW_MS] is USUALLY the ordering artefact rather than
// a real dead control — usually, not always: see the header comment for the
// measured false negative (a dead control clicked shortly after an unrelated
// mutation). Inclusive of 0 — 15 of the 38 measured artefacts sit exactly
