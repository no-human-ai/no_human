...
test("a network rejection from submitEmail surfaces the exact pinned offline banner text, byte-identical to the wizard's existing banner", async () => {
  // Same contract onboardingOffline.test.mjs pins for every other step. This
  // drives the rejection THROUGH submitEmail rather than calling
  // offlineBanner() directly: the earlier version asserted only on the
  // banner helper, so a submitEmail that never called the API left it green
  // while every other email-path test went red. Routing it through the real
  // path is what makes this test die with its siblings.
  const registerOnboardingEmail = async () => {
  ...
  assert.equal(threw, true, "submitEmail must reject so the wizard can classify it");
