...
// Derive the wizard's real step keys from Onboarding.jsx's own BASE_STEPS
// array, rather than hardcoding a list here — a hardcoded list is exactly
// what let FUNNEL_STEPS silently drift out of sync with the wizard when
// "email" and "discord" were added to BASE_STEPS after this file was
// written (Task c4873936). This is the derivation, not a duplicate literal.
function wizardStepKeysFromSource(src) {
  const m = src.match(/const BASE_STEPS = \[([\s\S]*?)\n\];/);
  assert.ok(m, "expected a BASE_STEPS = [ ... ]; array literal in Onboarding.jsx");
  const body = m[1];
  const keys = [];
  const keyRe = /\{\s*key:\s*"([^"]+)"/g;
  let km;
  while ((km = keyRe.exec(body)) !== null) keys.push(km[1]);
  assert.ok(keys.length > 0, "expected at least one { key: \"...\" } entry in BASE_STEPS");
  return keys;
}

test("FUNNEL_STEPS matches Onboarding.jsx's BASE_STEPS, in order", () => {
  const wizardSteps = wizardStepKeysFromSource(onboardingJsx);
  assert.deepEqual(
    FUNNEL_STEPS,
    wizardSteps,
    "onboardingFunnel.js's FUNNEL_STEPS must be derived from / kept equal to " +
      "Onboarding.jsx's BASE_STEPS keys, in the same order — a hardcoded copy " +
      "here silently drops steps the wizard actually shows (e.g. 'email', " +
      "'discord') from the funnel."
  );
});
...
test("all real steps each report once, in FUNNEL_STEPS order", () => {
  const sent = [];
  const report = makeStepReporter((step) => { sent.push(step); });
