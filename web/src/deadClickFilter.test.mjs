...
// The finite-stamp guard is load-bearing: posthog ships these as numbers, but
// a non-number that still compares (a numeric string, a Date) would make the
// gap arithmetic coerce and could flip an event's verdict. Landing review of
// 55916ece measured that removing `Number.isFinite` left the suite green, so
// pin it here — each case below is an artefact-shaped gap that must NOT be
// dropped, because the stamps are not finite numbers.
test("non-finite stamps are never treated as the race artefact", () => {
  const numericString = deadClick({
    $dead_click_event_timestamp: String(EVENT_TS),
    $dead_click_last_mutation_timestamp: String(EVENT_TS - 3),
  });
  assert.equal(deadClickBeforeSend(numericString), numericString);

  const dates = deadClick({
    $dead_click_event_timestamp: new Date(EVENT_TS),
    $dead_click_last_mutation_timestamp: new Date(EVENT_TS - 3),
  });
  assert.equal(deadClickBeforeSend(dates), dates);

  const nan = deadClick({
    $dead_click_event_timestamp: Number.NaN,
    $dead_click_last_mutation_timestamp: EVENT_TS - 3,
  });
  assert.equal(deadClickBeforeSend(nan), nan);

  const infinite = deadClick({
    $dead_click_event_timestamp: Number.POSITIVE_INFINITY,
    $dead_click_last_mutation_timestamp: Number.POSITIVE_INFINITY,
  });
  assert.equal(deadClickBeforeSend(infinite), infinite);

  // Positive control: the same shape with real numbers IS dropped.
  assert.equal(
    deadClickBeforeSend(
      deadClick({
        $dead_click_event_timestamp: EVENT_TS,
