...
// in desktop/mainUpdateLast.test.mjs ("an update event with no subscriber
// is still retained" and friends), which exercises the real IPC
...
test("the shell calls getLastUpdate and writes its payload into state", () => {
  ...
  // Rendered on the board page only, immediately below `.nh-main-bar` and
  // ahead of the board's own content — see MAJOR-B. Anchored on the actual
  // `className="nh-main-bar"` JSX attribute (not the bare token, which also
  // appears in this file's own explanatory comments and would let the
  // assertion pass even if the notice moved elsewhere), with the window
  // tightened to just above the measured real gap so a comment sneaking back
  // between the two blocks fails this test.
  const flowSite = appSrc.match(/className="nh-main-bar"[\s\S]{0,500}?\{page === "board" && updateBar && \(/);
  assert.ok(flowSite,
    "the update notice must be rendered immediately after .nh-main-bar, gated on page === \"board\"");
});

test("the notice renders only inside the board page gate", () => {
  assert.match(appSrc, /\{page === "board" && updateBar && \(/,
    "the notice render must be gated on page === \"board\" so it never lands on Backlog/Done/Failed/Stats/About, under Windows' titleBarOverlay strip");
  assert.doesNotMatch(appSrc, /\{updateBar && \(/,
    "there must be no ungated render of the notice left anywhere in App.jsx");
});
