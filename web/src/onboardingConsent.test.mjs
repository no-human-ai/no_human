...
test("the 6 base steps are untouched and nothing is appended after summary", () => {
  ...
  assert.deepEqual(
    ...
    // "email" was ADDED 2026-09-12 (operator decision: required, not
    // skippable) — unlike telemetry consent, this is not a removed step
    // creeping back; see onboardingEmailStep.test.mjs for its own coverage.
    ["welcome", "email", "repos", "projects", "integrations", "summary"],
