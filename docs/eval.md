...
spec — `nh bench run --trials N` (`src/no_human/cli/commands.py:bench_run:8103`), with a
spec-major fan-out (`:bench_run:8254`) and `(task_id, trial)` as the checkpoint
identity (`:bench_run:8132`) so a resumed multi-trial run cannot double-count a spec.
