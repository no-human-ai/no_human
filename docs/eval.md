...
spec — `nh bench run --trials N` (`src/no_human/cli/commands.py:bench_run:7742`), with a
spec-major fan-out (`:bench_run:7893`) and `(task_id, trial)` as the checkpoint
identity (`:bench_run:7771`) so a resumed multi-trial run cannot double-count a spec.
