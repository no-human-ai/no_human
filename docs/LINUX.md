...
| 7 | No auto-update | `nhCanAutoUpdate` is `false` on every platform today; electron-updater on Linux supports AppImage only; `latest-linux.yml` ships as a release asset: it enables in-app update checks to report available versions, while the install path stays refused | decision (§2.1) |
