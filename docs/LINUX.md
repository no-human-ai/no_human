...
| 7 | No auto-update | `nhCanAutoUpdate` is `false` on this unsigned Linux build (as on the unsigned Windows build; the signed **and** notarized macOS build stamps it `true` and does install); electron-updater on Linux supports AppImage only; `latest-linux.yml` ships as a release asset: it enables in-app update checks to report available versions, while the install path stays refused | decision (§2.1) |
...
9. `sha256sum` → `SHA256SUMS-linux.txt`. Two run artefacts: the EVIDENCE
   ...
   `latest-linux.yml` — enables in-app update checks to report available versions;
   ONLY on a `workflow_dispatch` run with `linux_release=true`, 7 days). The split
