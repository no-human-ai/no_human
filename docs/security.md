...
<<<<<<< HEAD
  (`cli/commands.py:merge_stack_run:3224`). This is *your* command, not the agent's — an agent
=======
...
<<<<<<< HEAD
  (`_refuse_agent_gate_act`, `cli/commands.py:approve:5594`, `:merge_stack_run:3194`) refuse before
=======
...
- **The desktop app checks GitHub Releases at startup**, once a day
  ...
  no_human` — `desktop/electron-builder.config.cjs:371`). It never downloads on its own
