...
| 16 | A subprocess's text output is always decoded `encoding="utf-8", errors="replace"`, never bare `text=True` | On POSIX, no explicit `encoding=` falls back to the interpreter's preferred encoding and an undecodable byte raises `UnicodeDecodeError` where the caller can see it. On Windows the SAME fallback decode happens inside `subprocess`'s background reader thread (`_readerthread`); the exception kills that thread silently and `Popen.communicate()` returns `None` for the stream instead of raising anywhere — every `.strip()`/`.splitlines()` downstream then either crashes on `None` or is swallowed by a broad `except`. Enforced flat-zero across `src/no_human` by `tests/test_subprocess_decodes_utf8.py`, which also reduces the Windows mechanism to a real background thread on any host to prove the reduction is faithful |
...
| `test_try_kill_still_signals_on_posix` | `signal.SIGKILL` missing on Windows → `AttributeError` at `cli/commands.py:_try_kill:7637` — the exact shape the neighbouring `test_stop_path_never_names_sigkill_at_module_scope` exists to fence |
